...
The OCSF mapping names the API activity only when the operation states it:
an HTTP verb. Tool names and JSON-RPC method paths get `Other` with the
operation in `activity_name`, as the schema prescribes when no enum member
fits; guessing read/write intent from the shape of a name would misclassify a
tool such as `cleanup_stale_records` as a read.
...
Tool decisions no longer place the provisioning source (`tool_source`) into
`api.service.name`, which other producers use for the MCP server name; the
tool name stays in `api.operation` until the proposed OCSF `ai_tool` object
provides a home for tool identity.
...
Exec-server HTTP spans carry gateway-grade data — host, method, and usually a
response status — and map to HTTP Activity with `http_request`,
`http_response`, and `dst_endpoint` populated. Model websocket connections
report no host, method, or status, only the wire API and path, so they stay
unmapped; that is a vendor gap.
...
taken from the `mcp_server` attribute, the discovery span's server name, or
the `mcp__`-prefixed tool-name convention. This is the package's declared
