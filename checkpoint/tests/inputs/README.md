...
| `checkpoint-expanded.ndjson` `threat-1` | Synthetic from documented IPS/threat-prevention fields | JSON/NDJSON | IPS | `attack` and `protection_name` | `checkpoint.firewall.threat` | Detection Finding |
...
| `checkpoint-expanded.ndjson` `email-1` | Synthetic from documented MTA/Anti-Spam fields | JSON/NDJSON | MTA | `from`, `to`, and `email_subject` | `checkpoint.firewall.email` | Email Activity |
...
| `checkpoint-expanded.ndjson` `audit-1` | Synthetic from documented SmartConsole/Web UI fields | JSON/NDJSON | SmartConsole | administrator, client IP, action, and object fields | `checkpoint.management.audit` | Entity Management |
