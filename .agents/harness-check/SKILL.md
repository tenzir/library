...
working directory. For every new invocation, run `scripts/new-run.sh
claude|codex` once. Save its returned absolute path as `<run>` and pass
...
2. Run `scripts/inventory.sh --agent claude` or `--agent codex` according to
   the harness executing this skill. Do not rely on home-directory detection
...
5. Read and execute `reference/agent-probes.md` using the current harness's
   native tools. Append every outcome through `scripts/record.sh`.
6. Read `reference/child-sessions.md` and run
   `bash scripts/run-child.sh --agent claude|codex` for the current product to
   generate startup-only, plugin, hook, and MCP activity.
...
Determine all applicable manual steps, then run
`bash scripts/probe-operator.sh prepare <id>...` once before presenting any of
them. Present exactly one action and end the turn. On each user reply, retain
the observed result in the conversation and present the next single action
without running Bash. Never reveal the remaining checklist. After the last
manual result, run one command only:
`bash scripts/probe-operator.sh batch-mark <id>=PASS|FAIL|SKIP ...`.
...
Run `scripts/summarize.sh` and report its effective counts. The latest result
