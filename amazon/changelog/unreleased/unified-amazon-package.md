...
- `amazon::vpc_flow::ocsf::normalize` - Normalize to OCSF Network Activity (class 4001)
...
The VPC Flow Log parsers expect a field containing the flow log line, defaulting
to `message`, and automatically convert `start`/`end` fields from Unix epoch
seconds to proper timestamps.
